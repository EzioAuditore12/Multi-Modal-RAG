...
export function ChatToolbar({ children, className, ...props }: ChatToolbarProps) {
  return (
    <div className={cn('bg-background sticky bottom-0 p-2 pt-0', className)} {...props}>
      <div className={cn('rounded-md border px-3 py-2', 'flex flex-wrap items-start gap-x-2')}>
        {children}
      </div>
    </div>
  );
